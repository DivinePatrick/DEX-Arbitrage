...
const lookForTriTrade = async () => {
  ...
  try {
    ...
    if (!config.routes.length > 0) {
      fs.appendFile(`./data/${network}RouteLog.txt`, `["${targetRoute.router1}","${targetRoute.router2}","${targetRoute.router3}","${targetRoute.token1}","${targetRoute.token2}","${targetRoute.token3}"],`+"\n", function (err) {});
    }
...
const triTrade = async (router1,router2,router3,baseToken,token2,token3,amount) => {
  ...
  try {
    ...
    const tx = await arb.connect(owner).dualDexTrade(router1, router2, router3, baseToken, token2, token3, amount); //{ gasPrice: 1000000000003, gasLimit: 500000 }
    await tx.wait();
    inTrade = false;
    await lookForTriTrade();
  } catch (e) {
